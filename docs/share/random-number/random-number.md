...
在选取素数模乘同余发生器的参数 $M$ 和 $a$ 的时候，可以取 $M$ 为小于 $2L$ 的最大素数 \($L$为计算机整数表示的尾数位数\)，取 $a$ 为 $M$ 的素元，这样保证周期 $T = M − 1$。$a$ 应尽可能大，$a$ 的二进制表示尽可能无规律。这样在一个周期内可以得到 $1, \, 2, \, ..., \, M − 1$ 的一个排列。初值 $x\_0$ 可以取 $1, \, 2, \, ..., \, M − 1$ 中任一个。
